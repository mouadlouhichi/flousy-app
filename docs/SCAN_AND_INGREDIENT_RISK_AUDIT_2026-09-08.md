...
### Product-owner direction (2026-09-09, scan-camera restoration and risk-rank enhancement)

A further product-owner review of the remediated build directed two restorations and made risk-rank enhancement the development priority. Implemented and regression-covered in this revision:

- **Original scan-camera interaction restored.** The shared `BarcodeScannerPanel` again renders exactly like the pre-audit scanner in every decoder mode: the corner-bracket scan frame with the animated sweeping line, the alignment hint, and the zoom control are always visible while the camera runs. The conditional hiding of the frame/hint behind native-ROI state and the gating of the zoom pill behind hardware-zoom capability were reverted as product-UX regressions; the added camera-picker dropdown was removed. The zoom control works on every device again (hardware track zoom where exposed, CSS digital zoom otherwise, default 2× framing, range 1–8×, 0.5 steps). Decoder-internal ROI cropping, the one-acquisition/one-action lifecycle, and differentiated camera-error copy are retained; a source contract test now pins the visible treatment (`tests/scanner-lifecycle.test.ts`, "shared scanner camera UX contract").
- **Add-expense scan-product entry verified and pinned.** The Pro "Scan product barcode" entry in the Add-Expense modal, the shared scanner, the canonical resolution path, and the product-attachment confirmation are present and unchanged; the report of a missing entry matched a stale Vercel preview of an intermediate remediation commit (the final head was not deployed because of the account build-rate limit). A source contract test now fails any future build that removes the entry or its shared scanner.
- **Second product-owner review (2026-09-09, risk direction + camera liveness).** Two further directions were implemented on top of the restoration:
  - **Risk-score presentation.** The owner read a higher number as higher risk; the previous presentation (100 = cleanest) was inverted for users. Every surface (glance banner, line chips, bill, food ring) now displays `100 − index` as a **risk score** (0 = lowest listed risk, 100 = highest) with a scale caption and risk-worded bands (Low → High risk). Engine semantics and stored history are unchanged; the inversion is presentation-only and applies consistently to old and new records.
  - **Vague-label rubric (`2026-09-food-v5`).** A reproduced same-product comparison had the vaguer label scoring **better** (94 vs 85) because generic declarations ("flavor enhancers", "food acid", "protein", "color") carried no penalty. Each distinct unspecified class declaration now deducts 15 points (watch-level) unless its E-range is enumerated elsewhere on the same label (bare "color" next to a declared E160b is one transparent declaration the parser splits, not hidden vagueness). The Pringles vague-twin regression now reads 55/caution instead of 94/good, and the breakdown ranks the vague declarations as risk drivers. A truncated label leaving a stray `)` (the reported `chili extract)`) no longer leaks the bracket into the identity; the cosmetic parse stays conservatively invalid.
  - **Camera stays live after a scan.** Acceptance now pauses only the decoders: the stream, preview, torch and zoom survive a scan, a consumer gate (resolving/pending), and re-arm without a camera restart — the original main-branch behaviour. Full teardown remains on unmount, the stop button, and visibility loss. One-acquisition/one-action is retained: no decode happens until re-arm, and the same stationary code is still suppressed.
  - **Continuous scanning and POS quantity increment (final parity round).** The courses screen no longer gates the scanner `enabled` off during resolving, the pending card, or a notice — once enabled, the camera decodes continuously, matching the original scanner. `accept()` now re-arms a new generation on the still-live stream by itself (no consumer `enabled` flip is needed to resume), while the one-claim mutex and a 1.5 s same-code re-trigger window still prevent a held barcode from machine-gunning accepts. With a pending card open, scanning the **same** GTIN increments its quantity (+1, localized “{name} +1 (×{qty})” notice) instead of failing, and scanning a **different** product is refused with a “finish or skip the current product” warning instead of silently replacing the card. The manual code lookup no longer stops the camera either. Source contracts pin the in-`accept` re-arm, the re-trigger window, the always-on course panel, and the pending-card increment.
  - **Stop/start camera fix, one-row quantity step, and wrong-result reports.** The header stop/start button had stopped working: the keep-alive refactor left the stream marked live after teardown, so the toggle stayed on “Stop” and the camera could never be restarted from the panel; teardown now resets the preview state and Start re-acquires cleanly (contract-pinned). The pending card's quantity, price and Add controls render as one non-wrapping row — the price field flexes into the remaining space instead of wrapping on narrow screens. A new “Wrong result?” report flow on the pending card lets a shopper flag a bad lookup (optional note, localized sent/offline confirmation); every report is mirrored to an on-device queue (`smartjib.productReports.v1`, capped at 50) and written to Firestore `users/{uid}/productReports` when signed in — owner-scoped, write-once, bounded-shape rules with Firestore-emulator coverage, reviewable through an admin path that bypasses the rules.
- **Risk-rank enhancement (`ingredient-evidence-v3`, `2026-09-food-v4`).**
  - Transparency: every assessed cosmetic row now records its exact index `deduction`; the scan glance ranks up to three "main risk drivers" (name, localized tier, point cost) under the index banner, only when the numeric index is available. The food panel gained the mirror-image "what moved the label score" breakdown computed by `foodGradeDrivers`, which reuses the exact `foodLabelGrade` rubric so the breakdown can never disagree with the ring.
  - Coverage: the cosmetic identity-alias table gained verified trade-name and French/Arabic aliases (aloe vera, witch hazel, rose water, honey, colloidal oatmeal, green tea, lavender/tea-tree oils, vitamin synonyms, glycerine végétale, huile d'olive, acide hyaluronique, Arabic oil/butter terms). The food corpus gained Moroccan/North-African market rows (spices, dates, figs, mackerel, vinegar, coffee, chicory, malt extract) and Arabic label matching via a Unicode-aware fold; a 59-term Moroccan-market probe improved from 35 to 54 recognized terms, with generic class declarations (`conservateur`, `antioxydant`, `stabilisant`, `édulcorant`) deliberately still unresolved identities carrying explicit unspecified-class chips.
  - The engine and corpus version bumps create a new client cache identity, so previously cached analyses re-run under the expanded recognition.
...
- The continuous-scan/POS-quantity revision passed `npm run check` (lint, both TypeScript checks, **827 unit/integration + 63 render tests**) and a production build of all 55 static pages locally on 2026-09-09.
- The camera stop/start fix, one-row quantity step, and wrong-result report revision passed `npm run check` (lint, both TypeScript checks, **833 unit/integration + 63 render tests**) and a production build of all 55 static pages locally on 2026-09-09.
