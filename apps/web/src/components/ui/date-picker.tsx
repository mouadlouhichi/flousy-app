...
function displayDate(value: string, locale: string | undefined, emptyLabel: string): string {
...
export function DatePicker({
  ...
  const { messages: m, t, language, intlLocale, isRTL } = useLanguage();
  const [open, setOpen] = useState(false);
  const errorId = useId();
  const selectedDate = useMemo(() => dateFromInputValue(value), [value]);
  const activeLocale = locale || intlLocale;
  const selectedLabel = useMemo(
    () => displayDate(value, activeLocale, m.datePicker.chooseDate),
    [value, activeLocale, m.datePicker.chooseDate],
  );
