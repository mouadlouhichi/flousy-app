...
export function MemberBadges({ label = 'Paid by', value, onChange }: MemberBadgesProps) {
  const { payers } = useHousehold();
  return <div className="flex flex-col gap-1.5">
    {label && <label className="text-[11px] font-extrabold tracking-wider text-on-surface-variant uppercase">{label}</label>}
    <div role="group" aria-label={label} className="flex flex-wrap gap-2 py-0.5">
      {payers.map((member) => { const isActive = value === member.id || value === member.label;
        return <button key={member.id} type="button" aria-pressed={isActive} onClick={() => onChange(member.id, member.label)} className={`flex items-center gap-1.5 rounded-full py-1.5 pl-1.5 pr-3 text-[12px] font-semibold transition-all ${isActive ? 'bg-primary text-on-primary shadow-sm' : 'border border-outline-variant bg-surface-container-lowest text-on-surface hover:bg-surface-variant/40'}`}>
