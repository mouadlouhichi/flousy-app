...
export function MemberBadges({ label, value, onChange }: MemberBadgesProps) {
  const { payers, myMemberId } = useHousehold();
  const { messages: m } = useLanguage();
  const visibleLabel = label || m.modals.expense.householdMember;
  // Nothing to choose when the only payer is the signed-in member: a lone
  // "Me" chip under a "Household Member" heading reads like a missing
  // option rather than a default.
  if (payers.length <= 1) return null;
  // An expense saved with the member's own roster id is a "Me" expense.
  const current = myMemberId && value === myMemberId ? 'self' : value;
  return <div className="flex flex-col gap-1.5">
    ...
    <div role="group" aria-label={visibleLabel} className="flex flex-wrap gap-2 py-0.5">
      {payers.map((member) => { const isActive = current === member.id || current === member.label;
