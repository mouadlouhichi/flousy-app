...
function contrastOn(hex) {
  ...
  return inkContrast >= whiteContrast ? color.forestDeep : color.white;
}
...
function brand(args, width, inverted = false, y = 64) {
  text(args, 'SMARTJIB', 72, y, {
    size: 24,
    fill: inverted ? color.lime : color.forest,
    family: font.body,
    weight: 750,
    kerning: 1.4,
  });
  rect(args, 238, y + 2, 346, y + 37, 18, inverted ? '#1A4F48' : color.mint);
  ...
  sparkle(args, width - 93, y + 25, 33, inverted ? color.limeDeep : color.lime);
}
...
function footer(args, inverted = false, width = 1080, y = 1280) {
  ...
  text(args, 'BUDGET SIMPLE · MAROC', width - 365, y + 3, {
    size: 14,
    fill: inverted ? '#D6F0BD' : color.muted,
    family: font.body,
    weight: 650,
    kerning: 0.6,
  });
...
function button(args, label, x1, y1, x2, y2, fill = color.lime, labelFill = color.ink) {
...
function makeCalmFeedFr() {
  ...
  const a = base(1080, 1350, color.background);
  circle(a, 970, 112, 245, color.mint);
  sparkle(a, 860, 250, 45, color.limeDeep);
  brand(a, 1080);
  ...
  rect(a, 72, 492, 1008, 1055, 42, color.forest);
  text(a, 'TON PLAN DE CE MOIS-CI', 118, 543, { size: 17, fill: color.lime, weight: 700, kerning: 1 });
  ...
  rows.forEach(([number, title, detail], index) => {
    ...
    const marker = [color.lime, color.mint, color.sage][index];
    circle(a, 137, y + 22, 24, marker);
    text(a, number, 123, y + 6, { size: 16, fill: contrastOn(marker), weight: 750 });
    text(a, title, 190, y, { size: 27, fill: color.white, weight: 700 });
    text(a, detail, 408, y + 4, { size: 22, fill: '#D6F0BD' });
  });
  button(a, 'Commencer mon budget', 72, 1100, 445, 1175);
  text(a, 'Pas besoin d’un budget parfait.', 485, 1121, { size: 22, fill: color.forest, weight: 700 });
  footer(a);
  render(path, a);
  return path;
}
...
function makePurposeFeedFr() {
  ...
  const a = base(1080, 1350, color.mint);
  circle(a, 938, 100, 210, color.sage);
  brand(a, 1080);
  ...
  circle(a, 158, 610, 44, color.lime);
  circle(a, 158, 610, 20, 'none', color.forestDeep, 6);
  circle(a, 158, 610, 5, color.forestDeep);
  text(a, 'POUR QUOI ?', 240, 542, { size: 16, fill: color.forest, weight: 750, kerning: 1 });
  ...
  circle(a, 158, 870, 44, color.forest);
  ...
  text(a, 'OÙ ?', 240, 802, { size: 16, fill: color.lime, weight: 750, kerning: 1 });
  ...
  button(a, 'Voir comment ça marche', 72, 1045, 470, 1120, color.forest, color.white);
  text(a, 'Le plan n’est pas le solde.', 512, 1066, { size: 22, fill: color.forest, weight: 700 });
  footer(a);
  render(path, a);
  return path;
}
...
function makePrivateFeedFr() {
  ...
  const a = base(1080, 1350, color.forest);
  circle(a, 930, 55, 235, '#1A4F48');
  brand(a, 1080, true);
  heading(a, ['Sans connexion', 'bancaire.'], 72, 185, { size: 91, fill: color.white });
  text(a, 'Tu choisis ce que tu veux suivre.', 75, 385, { size: 28, fill: color.lime });
  rect(a, 72, 485, 1008, 1005, 44, color.forestDeep, '#4F7F5B', 3);
  circle(a, 540, 678, 128, color.forestSoft);
  shield(a, 540, 668, 0.62);
  text(a, 'TES CHOIX · TES DONNÉES', 348, 846, { size: 22, fill: color.lime, weight: 750, kerning: 1 });
  ...
  button(a, 'Découvrir le fonctionnement', 72, 1060, 545, 1135, color.lime, color.ink);
  text(a, 'MANUEL · SIMPLE · À TON RYTHME', 588, 1081, { size: 15, fill: color.lime, weight: 700, kerning: 0.6 });
  footer(a, true);
  render(path, a);
  return path;
}
...
function makeCalmFeedAr() {
  ...
  const a = base(1080, 1350, color.forest);
  circle(a, 155, 105, 185, '#1A4F48');
  brand(a, 1080, true);
  arabicHeading(a, ['فلوسك بوضوح،', 'بلا ضغط.'], 1000, 175, 1080, { size: 96 });
  arabicText(a, 'خطّط بالدرهم وعلى حساب واقعك.', 1000, 438, 1080, { size: 27, fill: color.lime });
  rect(a, 72, 530, 1008, 1000, 44, color.background);
  const rows = [
    ['ضروريات', 'اللي خاصّك'],
    ['رغبات', 'اللي بغيتي'],
    ['توفير', 'للي جاي'],
  ...
  rows.forEach(([title, detail], index) => {
    ...
    const panel = [color.forest, color.secondary, color.limeDeep][index];
    const panelText = contrastOn(panel);
    rect(a, x1, 615, x1 + 250, 885, 30, panel);
    circle(a, x1 + 125, 690, 39, color.white);
    text(a, ['01', '02', '03'][index], x1 + 104, 674, { size: 22, fill: color.forest, weight: 750 });
    arabicText(a, title, x1 + 222, 755, 1080, { size: 28, fill: panelText, weight: 700 });
    arabicText(a, detail, x1 + 222, 807, 1080, { size: 19, fill: panelText });
  });
  arabicText(a, 'ما خاصكش تكون كامل، غير بدا.', 955, 930, 1080, { size: 24, fill: color.forest, weight: 700 });
  rect(a, 660, 1055, 1008, 1132, 39, color.lime);
...
function makeCalmStoryFr() {
  ...
  const a = base(1080, 1920, color.background);
  circle(a, 925, 120, 260, color.mint);
  ...
  rect(a, 72, 660, 1008, 1385, 48, color.forest);
  text(a, 'UN PLAN QUI BOUGE AVEC LA VRAIE VIE', 118, 720, { size: 18, fill: color.lime, weight: 700, kerning: 1 });
  const rows = [
    ['01', 'Prévoir', 'ce qui entre'],
    ['02', 'Répartir', 'besoins · envies · épargne'],
  ...
  rows.forEach(([n, title, detail], index) => {
    ...
    const marker = [color.lime, color.mint, color.sage][index];
    circle(a, 150, y + 25, 33, marker);
    text(a, n, 130, y + 6, { size: 22, fill: contrastOn(marker), weight: 700 });
    text(a, title, 230, y, { size: 37, fill: color.white, family: font.display, weight: 800 });
    text(a, detail, 230, y + 62, { size: 25, fill: '#D6F0BD' });
  });
  button(a, 'Commencer mon budget', 72, 1495, 480, 1580, color.lime, color.ink);
  text(a, 'Pas besoin d’un budget parfait.', 75, 1640, { size: 27, fill: color.forest, weight: 700 });
...
function makeCalmStoryAr() {
  ...
  const a = base(1080, 1920, color.forest);
  circle(a, 130, 110, 230, '#1A4F48');
  brand(a, 1080, true, 100);
  arabicHeading(a, ['فلوسك بوضوح،', 'بلا ضغط.'], 1000, 285, 1080, { size: 100 });
  arabicText(a, 'بدا بالمبلغ الحقيقي ديال هاد الشهر.', 1000, 555, 1080, { size: 28, fill: color.lime });
  rect(a, 72, 690, 1008, 1390, 48, color.background);
  const rows = [
  ...
  rows.forEach(([n, copy], index) => {
    ...
    const marker = [color.lime, color.mint, color.sage][index];
    circle(a, 920, y + 22, 34, marker);
    arabicText(a, n, 930, y + 2, 1080, { size: 24, fill: contrastOn(marker), weight: 700 });
  ...
  rect(a, 610, 1500, 1008, 1590, 45, color.lime);
  arabicText(a, 'نبدا ميزانيتي  ←', 955, 1518, 1080, { size: 28, fill: color.ink, weight: 700 });
  arabicText(a, 'ما خاصكش تكون كامل، غير بدا.', 1000, 1650, 1080, { size: 27, fill: color.lime, weight: 700 });
...
function makePrivateLandscape() {
  ...
  const a = base(1200, 628, color.forest);
...
function makeEmailHeader() {
  ...
  const a = base(1200, 480, color.background);
  circle(a, 1100, 55, 235, color.mint);
  circle(a, 1005, 400, 180, color.mint);
  sparkle(a, 1060, 150, 44, color.limeDeep);
  text(a, 'SMARTJIB', 60, 50, { size: 22, fill: color.forest, weight: 750, kerning: 1.4 });
